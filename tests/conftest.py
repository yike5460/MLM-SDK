...
from mlm import Mlm, AsyncMlm, DefaultAioHttpClient
from mlm._utils import is_dict
...
logging.getLogger("mlm").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Mlm]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Mlm(base_url=base_url, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncMlm]:
    ...
    async with AsyncMlm(base_url=base_url, _strict_response_validation=strict, http_client=http_client) as client:
        yield client
